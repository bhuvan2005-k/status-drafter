...
public class StatusService {
    ...
    public String exportUserToCsv(String username) {
        User user = userRepository.findByUsername(username)
                .orElseThrow(() -> new RuntimeException("User not found"));
        List<StatusRecord> records = statusRepository.findAllByUserIdAndStatusDateBetween(
                user.getId(), LocalDate.now().minusDays(30), LocalDate.now());
        
        StringBuilder csv = new StringBuilder("Date,Yesterday,Today,Blockers,Hours\n");
        for (StatusRecord r : records) {
            csv.append(String.format("%s,\"%s\",\"%s\",\"%s\",%s\n",
                    r.getStatusDate(),
                    r.getParsedYesterday(),
    ...
    public String exportUserToTxt(String username) {
        User user = userRepository.findByUsername(username)
                .orElseThrow(() -> new RuntimeException("User not found"));
        List<StatusRecord> records = statusRepository.findAllByUserIdAndStatusDateBetween(
                user.getId(), LocalDate.now().minusDays(30), LocalDate.now());
        
        StringBuilder txt = new StringBuilder("MY STATUS HISTORY - " + user.getFullName() + "\n");
        txt.append("Last 30 Days\n\n");
        for (StatusRecord r : records) {
            txt.append("DATE: ").append(r.getStatusDate()).append("\n");
            txt.append("YESTERDAY: ").append(r.getParsedYesterday()).append("\n");
            txt.append("TODAY: ").append(r.getParsedToday()).append("\n");
            txt.append("BLOCKERS: ").append(r.getParsedBlockers()).append("\n");
            txt.append("HOURS: ").append(r.getTotalHoursSpent()).append("\n");
