...
public class StatusController {
    ...
    @GetMapping(value = "/export/my/csv", produces = "text/csv")
    public ResponseEntity<String> exportMyCsv(Principal principal) {
        return ResponseEntity.ok()
                .header("Content-Disposition", "attachment; filename=my_status_history.csv")
                .body(statusService.exportUserToCsv(principal.getName()));
    }

    @GetMapping(value = "/export/my/txt", produces = MediaType.TEXT_PLAIN_VALUE)
    public ResponseEntity<String> exportMyTxt(Principal principal) {
        return ResponseEntity.ok()
                .header("Content-Disposition", "attachment; filename=my_status_history.txt")
                .body(statusService.exportUserToTxt(principal.getName()));
    }
