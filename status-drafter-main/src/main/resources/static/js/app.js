...
let currentUserRole = 'USER'; // Store user role
...
async function fetchUserInfo() {
    try {
        ...
        currentUserRole = user.role; // Save role
...
function downloadReport(format) {
    // Admins get team export from daily report, Users get their personal history
    if (currentUserRole === 'ROLE_ADMIN') {
        window.location.href = `/api/v1/status/export/${format}`;
    } else {
        window.location.href = `/api/v1/status/export/my/${format}`;
    }
}
